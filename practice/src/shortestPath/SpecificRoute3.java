	...
	public static boolean Dijkstra(int startIndex, int vertexA, int vertexB) {
		...
		while(!prioriQueue.isEmpty()) {
			...
			for (int i = 0; i < graph.get(nowVertex.vertex).size(); i++) {
				Vertex temp =  graph.get(nowVertex.vertex).get(i);
				if(!visitiedMap[temp.vertex] ){
					if(distanceMap[temp.vertex] > distanceMap[nowVertex.vertex]+temp.weight){
						distanceMap[temp.vertex] = distanceMap[nowVertex.vertex]+ temp.weight;
						Vertex nextVertex = new Vertex(temp.vertex, distanceMap[temp.vertex]);

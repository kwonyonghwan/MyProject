	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (test_case = 1; test_case <= T; test_case++) {
			...
			while (!startQueue.isEmpty()) {
				boolean[][][] visitiedMap = new boolean[N][N][2];
		...
		if (nowPosition.digCount > 0) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (graph[nextPosition.positionI][nextPosition.positionJ] > -1
						...
						&& !visitiedMap[nextPosition.positionI][nextPosition.positionJ][1]) {
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < k + 1; j++) {
					...
					if (graph[nextPosition.positionI][nextPosition.positionJ] > -1
							...
							&& !visitiedMap[nextPosition.positionI][nextPosition.positionJ][1]	&& !visitiedMap[nextPosition.positionI][nextPosition.positionJ][0]) {
						
						graph[nextPosition.positionI][nextPosition.positionJ] -= j;
						
						nextPosition.digCount =	 (j>0)?1:0;
						dfs(nextPosition, visitiedMap, graph, k, length);
						graph[nextPosition.positionI][nextPosition.positionJ] += j;
